...
class GitService:
    ...
    def checkout_and_pull(self, branch_name: str) -> bool:
        """Checkout a branch and reset to latest from origin"""
        console_helper.show_info(f"Checking out {branch_name}...")
        success, _ = self._run_git("checkout", branch_name)
        if not success:
            console_helper.show_error(f"Could not checkout {branch_name}")
            return False

        console_helper.show_info(f"Pulling latest from origin/{branch_name}...")
        success, _ = self._run_git("fetch", "origin")
        if not success:
            console_helper.show_warning("Could not fetch from origin")

        success, _ = self._run_git("reset", "--hard", f"origin/{branch_name}")
        if not success:
            console_helper.show_error(f"Could not sync {branch_name} with remote")
            return False

        console_helper.show_success(f"Up to date with origin/{branch_name}")
        return True

    def create_branch(self, branch_name: str, from_branch: str = "qa") -> bool:
